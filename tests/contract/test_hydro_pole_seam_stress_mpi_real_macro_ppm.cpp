...
int main(int argc, char** argv) {
  ...
  try {
    ...
    constexpr std::size_t kShellIndex = 5u;
    ...
    if (!rho_exchange.is_complete(kThetaCells, kPhiCells, kGhostLayers)) {
      throw std::runtime_error(rho_exchange.report_line);
    }
    if (!mom_r_exchange.is_complete(kThetaCells, kPhiCells, kGhostLayers)) {
      throw std::runtime_error(mom_r_exchange.report_line);
    }
    if (!mom_theta_exchange.is_complete(kThetaCells, kPhiCells, kGhostLayers)) {
      throw std::runtime_error(mom_theta_exchange.report_line);
    }
    if (!mom_phi_exchange.is_complete(kThetaCells, kPhiCells, kGhostLayers)) {
      throw std::runtime_error(mom_phi_exchange.report_line);
    }
    if (!e_fluid_total_exchange.is_complete(kThetaCells, kPhiCells, kGhostLayers)) {
      throw std::runtime_error(e_fluid_total_exchange.report_line);
    }
    if (!e_electron_exchange.is_complete(kThetaCells, kPhiCells, kGhostLayers)) {
      throw std::runtime_error(e_electron_exchange.report_line);
    }
    ...
    if (rank == 0) {
      ...
      const dec3d::core::StageContext context{
          ...
          "p1-v0.1",
          "mesh:p1.case4.pole-stress-mpi-real-macro-ppm",
          "ownership:rank0",
          "diagnostics:p1.hydro.case4.mpi-real-macro-ppm"};
      ...
      DEC3D_CHECK(HasDiagnosticCode(single_rank_stage_result.diagnostics, "p1.hydro.stage.macro_zoning"));
      DEC3D_CHECK(HasDiagnosticCode(single_rank_stage_result.diagnostics, "p1.hydro.reconstruction.ppm.executed"));
      DEC3D_CHECK(HasDiagnosticCode(single_rank_stage_result.diagnostics, "p1.hydro.reconstruction.ppm.direction.theta"));
      DEC3D_CHECK(HasDiagnosticCode(single_rank_stage_result.diagnostics, "p1.hydro.reconstruction.ppm.direction.phi"));
      ...
      mpi_parity.macro_zoning_executed = true;
      mpi_parity.reconstruction_ghost_layers = kGhostLayers;
      mpi_parity.reconstruction_mode = "macro_zoning_mpi_real_angular_ppm";
      mpi_parity.macro_zoning_mode = "mpi_real_angular_macro_ppm";
      ...
      const std::filesystem::path output_dir =
          "F:\\dec3d\\analysis\\output\\case4_pole_seam_stress_mpi_real_macro_ppm";
      ...
      DEC3D_CHECK(dec3d::hydro::WritePoleAdjacentSeamStressOutputs(
          ...
          &mpi_parity,
          &single_rank_budget,
          "case4_pole_seam_stress_mpi_real_macro_ppm"));

      const auto summary_text = dec3d::testsupport::ReadTextFile(output_dir / "summary.txt");
      DEC3D_CHECK(summary_text.find("case=case4_pole_seam_stress_mpi_real_macro_ppm") != std::string::npos);
      ...
      DEC3D_CHECK(summary_text.find("macro_zoning_executed=true") != std::string::npos);
      ...
      DEC3D_CHECK(parity_text.find("macro_zoning_executed=true") != std::string::npos);
      DEC3D_CHECK(parity_text.find("reconstruction_ghost_layers=3") != std::string::npos);
      DEC3D_CHECK(parity_text.find("reconstruction_mode=macro_zoning_mpi_real_angular_ppm") != std::string::npos);

      const auto seam_text =
          dec3d::testsupport::ReadTextFile(output_dir / "seam_polar_diagnostics.txt");
      DEC3D_CHECK(seam_text.find("max_seam_mom_theta_jump_after") != std::string::npos);
      DEC3D_CHECK(seam_text.find("max_seam_mom_phi_jump_after") != std::string::npos);
    }
