...
int main() {
  try {
    ...
    const std::filesystem::path output_dir =
        "F:\\dec3d\\analysis\\output\\case2_radial_wave_ale_on_off";
    ...
    DEC3D_CHECK(std::filesystem::exists(output_dir / "ale_off_radial_profile_t1.txt"));
    DEC3D_CHECK(std::filesystem::exists(output_dir / "ale_on_radial_profile_t1.txt"));
    DEC3D_CHECK(std::filesystem::exists(output_dir / "budget_residual_off.txt"));
    DEC3D_CHECK(std::filesystem::exists(output_dir / "budget_residual_on.txt"));
    ...
    DEC3D_CHECK(summary_text.find("case=case2_radial_wave_ale_on_off") != std::string::npos);
    ...
    DEC3D_CHECK(summary_text.find("ppm_executed=false") != std::string::npos);

    const auto moving_mesh_text = ReadTextFile(output_dir / "moving_mesh_diagnostics.txt");
    DEC3D_CHECK(moving_mesh_text.find("outer_face_displacement=") != std::string::npos);
    DEC3D_CHECK(moving_mesh_text.find("max_face_speed=") != std::string::npos);

    const auto budget_comparison = ReadTextFile(output_dir / "budget_residual_comparison.txt");
    DEC3D_CHECK(budget_comparison.find("mom_r_residual_difference=") != std::string::npos);
