...
int main() {
  try {
    ...
    first_order_options.apply_geometric_source = true;
    first_order_options.apply_radial_sweep = true;
    first_order_options.apply_theta_sweep = true;
    first_order_options.apply_phi_sweep = true;
    first_order_options.use_ppm_reconstruction = false;
    first_order_options.reconstruction_ghost_layers = 1u;
    first_order_options.apply_radial_ale_flux_correction = false;
    first_order_options.request_radial_ale_proposal = false;
    first_order_options.use_macro_zoning = false;

    const std::filesystem::path output_dir =
        "F:\\dec3d\\analysis\\output\\case_sedov_spherical_ppm_off_trial";
    ...
    if (!summary.is_complete()) {
      std::cerr << "sedov first-order summary incomplete\n";
      return 1;
    }
    ...
    if (!WriteSedovSphericalBlastOutputs(
            ...
            run.profile_history,
            &run.final_budget,
            "case_sedov_spherical_ppm_off_trial")) {
      std::cerr << "failed to write sedov first-order outputs\n";
      return 1;
    }
