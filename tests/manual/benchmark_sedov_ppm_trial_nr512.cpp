...
int main() {
  try {
    ...
    const std::filesystem::path output_dir =
        "F:\\dec3d\\analysis\\output\\case_sedov_spherical_ppm_trial_nr512";
    ...
    if (!summary.is_complete()) {
      std::cerr << "sedov ppm nr512 summary incomplete\n";
      return 1;
    }
    ...
    if (!WriteSedovSphericalBlastOutputs(
            ...
            run.profile_history,
            &run.final_budget,
            "case_sedov_spherical_ppm_trial_nr512")) {
      std::cerr << "failed to write sedov ppm nr512 outputs\n";
      return 1;
    }
