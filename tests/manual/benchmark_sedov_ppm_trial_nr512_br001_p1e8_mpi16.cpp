...
int main(int argc, char** argv) {
  ...
  try {
    ...
    if (rank_count != 16) {
      if (rank == 0) {
        std::cerr << "expected 16 MPI ranks\n";
      }
      MPI_Finalize();
      return 1;
    }
    ...
    const std::filesystem::path output_dir =
        "F:\\dec3d\\analysis\\output\\case_sedov_spherical_ppm_trial_nr512_br001_p1e8_mpi16";
    if (rank == 0) {
    ...
    while (time_s + 1.0e-16 < kFinalTime && step < kMaxSteps) {
      ...
      const dec3d::core::StageContext probe_context{
          ...
          "p1-v0.1",
          "mesh:p1.sedov.br001.mpi16",
          "ownership:rank" + std::to_string(rank),
          "diagnostics:p1.hydro.sedov.br001.mpi16"};
    ...
    if (rank == 0) {
      ...
      if (!summary.is_complete()) {
        std::cerr << "sedov mpi br001 summary incomplete\n";
        MPI_Finalize();
        return 1;
      }
      ...
      if (!dec3d::hydro::WriteSedovSphericalBlastOutputs(
              ...
              profile_history,
              &reduced_budget,
              "case_sedov_spherical_ppm_trial_nr512_br001_p1e8_mpi16")) {
        std::cerr << "failed to write sedov mpi br001 outputs\n";
        MPI_Finalize();
        return 1;
      }
